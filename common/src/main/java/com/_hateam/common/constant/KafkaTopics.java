package com._hateam.common.constant;

public class KafkaTopics {
    public static final String ORDER_CREATED = "order-created";
    public static final String DELIVERY_CREATED = "delivery-created";
    public static final String DELIVERY_STATUS_CHANGED = "delivery-status-changed";

    public static final String ORDER_CREATED_FOR_SLACK = "order-created-for-slack";
    public static final String DAILY_DELIVERY_NOTIFICATION = "daily-delivery-notification";
}