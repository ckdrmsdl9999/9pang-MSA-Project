...
public class HubRouteDto {
    ...
    // HubRoute 엔티티를 HubDto로 변환하는 정적 팩토리 메소드
    public static HubRouteDto hubToHubDto(HubRoute hubRoute) {
        return HubRouteDto.builder()
                .id(hubRoute.getId())
                .name(hubRoute.getName())
                .address(hubRoute.getAddress())
                .latitude(hubRoute.getLatitude())
                .longitude(hubRoute.getLongitude())
                .build();
    }
