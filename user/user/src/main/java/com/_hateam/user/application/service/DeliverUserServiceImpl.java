...
//        }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<DeliverUserResponseDto> getDeliverUsersByHubId(UUID hubId, UserPrincipals userPrincipals) {
        // 회사 권한은 자신의 허브에 속한 배송담당자만 조회 가능
        if (userPrincipals.getRole() == UserRole.COMPANY) {
            // 회사 사용자의 허브 ID 확인 로직 필요
            DeliverUser companyDeliverUser = deliverUserRepository.findByUser_UserId(userPrincipals.getId())
                    .orElseThrow(() -> new CustomForbiddenException("허브 정보를 찾을 수 없습니다."));

            if (!companyDeliverUser.getHubId().equals(hubId)) {
                throw new CustomForbiddenException("다른 허브의 배송담당자를 조회할 권한이 없습니다.");
            }
        }

        List<DeliverUser> deliverUsers = deliverUserRepository.findByHubId(hubId).stream()
                .filter(du -> du.getDeletedAt() == null)
                .collect(Collectors.toList());
